(module project-test mzscheme
  
  (require (lib "list.ss" "srfi" "1")
           (lib "cut.ss"  "srfi" "26"))
  
  (require (file "project.ss")
           (file "test-base.ss"))
  
  (provide project-tests)
  
  ;; binary-sequences : integer -> (list-of (list-of (U #t #f)))
  ...
  (define project-tests
    (test-suite
     ...
     (test-case
      "project+fold calls proc the correct number of times"
      (let ([num-calls
             (lambda (data)
               (let ([counter 0])
     ...
     (test-case
      "project+fold accumulates data correctly"
      (let ([cons+accum
             (lambda (data)
               (project+fold cons
                             null
     ...
     (test-case
      "project+for-each works as expected"
      (let* ([counter 0]
             [do-map
              (lambda (data)
                (set! counter 0)
