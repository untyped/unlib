(module yield-test mzscheme
  ...
  (define yield-tests
    (test-suite
     ...
     (test-case
      "a procedure that does not use yield can be called more than once"
     ...
     (test-case
      "a procedure that uses yield can be called more than once"
      (let ([proc (make-yieldable
                   (lambda (yield)
                     (lambda ()
                       (yield 123)
     ...
     (test-case
      "yieldable procedures can take more than one argument"
      (let ([proc (make-yieldable
                   (lambda (yield)
                     (lambda (a b)
                       (define-values (c d)
     ...
     (test-case
      "yieldable procedures can return more than one value"
      (let ([proc (make-yieldable
                   (lambda (yield)
                     (lambda (a b)
                       (define-values (c d)
     ...
     (test-case
      "yieldable syntax works as expected"
