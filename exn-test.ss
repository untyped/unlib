(module exn-test mzscheme
  ...
  (define exn-tests
    (test-suite
     ...
     ; test case for DEPRECATED syntax
     (test-case
      "raise-exn/append raises an exception (DEPRECATED)"
      (check-exn
       exn:fail:unlib?
       (lambda ()
         (raise-exn/append exn:fail:unlib
           "This is an exception. "
           "Oh yes it is."))))

     ; test case for DEPRECATED syntax
     (test-case
      "raise-exn/format raises an exception (DEPRECATED)"
      (check-exn
       exn:fail:unlib?
       (lambda ()
